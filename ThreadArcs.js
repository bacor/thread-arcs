 ...
 * ThreadArcs class.
 *
 * The underlying graph should be described by a list L of parent nodes.
 * That is, L[i] should be a list with the indices of all parents of i.
 * So if j is in L[i], then j --> i, which means that article i is a
 * reply to article j. 
 * 
 * Such a list is generally referred to as the connection list. The list
 * of children can be seen as the 'inverse' of that connection list.
 *
 * ## Basic options
 *
 * - `orientation`: `vertical` or `horizontal`
 * - `nodes`: a list of objects containing additional information about the
 * 		nodes. The tooltip corresponding to a node contains HTML that is 
 * 		generated by a function `tooltipHTML` based on the objects in the list
 * 		`nodes`.
 * - `tooltipHTML`: a function that receives an object from the `nodes` array 
 * 		and returns an string of HTML that will be loaded into the tooltip.
 * - `disableTooltip`: disables the tooltip 
 *
 * ## Advanced options
 * 
 * - `space`: the spacing between two points. Defaults to 25px.
 * - `maxArcHeight`: the maximum height of an arc. Defaults to 60px.
 * - `padding`: padding at the top/bottom (in vertical mode)  or left/right 
 * 		(in horizontal mode)
 * - `lambda`: parameter for the shape of the arcs. Defaults to 2/3
 * - `radius`: radius of the points
 * - `size`: height of the threadArcs (vertical mode) or width (horizontal mode)
 * - `axisPos`: position of the axis
 *
 * @constructor
 * @this {ThreadArcs}
 * @param {string} container 	id of HTML container element
 * @param {array} parents 		list of parent nodes per node	
 * @param {array} options
 */
var ThreadArcs = function(container, parents, options)  {
	options || (options={});
	
	// Fix binding
	if (!(this instanceof ThreadArcs)) {
		return new ThreadArcs(container, parents, options);
	    ...
	    this.children 		= this.invertConnList(parents);
	    this.N 				= this.parents.length;
		...
		this.space			= (options['space'] || 25);
		this.maxArcHeight	= (options['maxArcHeight'] || 60);
		this.padding		= (options['padding'] || this.space/2);
		this.lambda			= (options['lambda'] || 2/3);
		this.radius 		= (options['radius'] || 5);
		this.width 			= this.space * (this.N - 1) + 2 * this.padding;
		this.height 		= (options['size'] || this.maxArcHeight * 2.5);
		this.axisPos		= (options['axisPos'] || this.height/2);
		...
		this.getTooltipHTML = (options['tooltipHTML'] || function(node){
			return node;
		})
		...
		arcLengths = []
		for(i=0; i<this.N; i++){
			for(j = 0; j < this.children[i].length; j++) {
				arcLengths.push( Math.abs(this.children[i][j] - i) );
			}
		}
...
ThreadArcs.prototype.invertConnList = function(connList){
	...
	connList.forEach(function(connections, i){
		connections.forEach(function(j){
			invertConnList[Math.abs(j)].push(i);
		}) 
	})
	return invertConnList;
}
 ...
 */
ThreadArcs.prototype.getPredecessorsDepths = function(i, depths, parents, counter){
	// Only enter recursion if necessary
	//if(depths.indexOf(i) == -1) {
	if(counter >= this.N * (this.N -1 ) / 2){
		console.log('Error: the graph contains cycles.')
		depths[i] = 0
		return depths
	}
	if(depths[i] == undefined) {
		var parentDepths = [];
		parents[i].forEach(function(child){
			depths = this.getPredecessorsDepths(child, depths, parents, (counter+=1));
			parentDepths.push(depths[child]);
		}.bind(this));
 ...
 * @return {array}          depths
 */
ThreadArcs.prototype.getDepths = function(refresh, parents) {
	refresh || (refresh == false)
	parents || (parents = this.parents)
	if(this.depths != undefined && refresh != true) {
		return this.depths
	}
	...
	while( depths.indexOf(undefined) != -1) {
		...
		depths = this.getPredecessorsDepths(i, depths, parents, counter);
	}
	this.depths = depths
	return depths
}
...
ThreadArcs.prototype.draw = function() {
	...
	// Draw all arcs
	for( i = 0; i < this.N; i++ ) {
		for(k = 0; k < this.children[i].length; k++) {
			var j = this.children[i][k];
 ...
 * @param  {int} N      
 * @return {array}        array with N repetitions of object
 */
